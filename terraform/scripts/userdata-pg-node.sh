#!/bin/bash
# Runs once on first boot (cloud-init per-instance, not per-boot).
# Places app-traffic.sh for manual use after the cluster is fully set up.

cat > /home/ubuntu/app-traffic.sh << 'SCRIPT'
#!/bin/bash
...
chmod +x /home/ubuntu/app-traffic.sh
chown ubuntu:ubuntu /home/ubuntu/app-traffic.sh
